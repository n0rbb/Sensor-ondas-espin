...
void setup() {
  set_arm_clock(600000000); //Forzar el reloj a correr a 600MHz, 800MHz para overclock.
  FreqMeasure.begin();
...
void loop() {
  ...
  uint8_t c = 100;
  uint32_t ct;
  //int measurements = 10000; //Número de veces que voy a contar la frecuencia antes de mostrar un resultado por pantalla
  //int validmeasurements = 1;  
  //for (int i = 0; i <= measurements; i++){ //Voy a promediarlo todo ahora
  ct = FreqMeasure.read(); 
  //if (ct <= 150){ //No voy a medir por debajo de un mega
  //   fmeas += FreqMeasure.countToFrequency(ct);
  //   validmeasurements++;
  //  }
  //}
  //fmeas /= (validmeasurements);
  fmeas = FreqMeasure.countToFrequency(ct);
  ...
  //if(key=='/') rf_ena = 1 - rf_ena; Solo vamos a cambiar la frecuencia, no queremos activar ni desactivar en ningún caso. Tampoco vamos a modificar la potencia de salida del ADF
  //if(key == '+') { 
  //  out_pwr = (out_pwr+1)%4;
